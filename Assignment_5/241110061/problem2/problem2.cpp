...
uint64_t runs = 2;
/** Number of threaads */
uint64_t THREADS = 16; // 1, 2, 4, 8 ,16
...
atomic < uint64_t > completed_ops(0); 
...
void create_file(path pth,
  const uint32_t * data, uint64_t size) {
...
void* t_func(void* args) {
    t_parameters* t_params = (t_parameters*)args;
    uint32_t* stack_values = t_params->stack_values;

    while (true) {
        unsigned long long current_ops = completed_ops.fetch_add(1);
        if (current_ops >= NUM_OPS) {
            break;
        }
        ...
        if (prob < 0.5) {
            push(stack_values[total_push]);
            total_push++;
        } else {
            long long popped = pop();
            total_pop++;
            if (popped != 0) {
                // just cout vraible: popped to see
            }
        }
...
int main(int argc, char * argv[]) {
  ...
  cout << "NUM OPS: " << NUM_OPS << " ADD: " << ADD << " REM: " << REM <<
    " FIND: " << FIND <<" THREADS LAUNCHED: "<<THREADS<< "\n";
  ...
  //path random_values_file = cwd / "random_values_insert.bin";
  path path_insert_keys = cwd / "random_keys_insert.bin";
  path path_stack_values = cwd / "random_values_insert.bin";
  ...
  assert(std::filesystem::exists(path_stack_values));
  ...
  read_data(path_stack_values, ADD, tmp_values_insert);
  for (int i = 0; i < ADD; i++) {
  ...
  t_parameters t_params[THREADS];
  uint32_t * stack_values = new uint32_t[NUM_OPS];
  read_data(path_stack_values, NUM_OPS, stack_values);

  cout << endl;
  cout << "This below part is extended part of problem 1 for concurrent stacks" << endl;
  ...
  for (int i = 0; i < THREADS; ++i) {
    ...
    if (pthread_create( & threads[i], nullptr, t_func, & t_params[i]) != 0) {
      cerr << "Failed to create threads " << i << endl;
      exit(EXIT_FAILURE);
    }
  }
  ...
  for (int i = 0; i < THREADS; ++i) {
    if (pthread_join(threads[i], nullptr) != 0) {
      cerr << "Threads Failed in joining " << i << endl;
      exit(EXIT_FAILURE);
    }
  }
  ...
  cout << "Total time for " << NUM_OPS << " operations with " << THREADS << " threads: " << duration << " ms" << endl;
  cout << "Total push operations: " << total_push << endl;
  cout << "Total pop operations: " << total_pop << endl;
  //stack_out();
